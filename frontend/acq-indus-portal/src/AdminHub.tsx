  ...
  const services = [
    {
      ...
      description: "Interface de suivi temps réel destinée aux opérateurs. Inclut la cartographie simplifiée, le statut des ventilateurs et les alertes de l'IA.",
    ...
    {
      title: "ThingsBoard IoT Engine",
      description: "Espace d'administration pour provisionner les capteurs/ventilateurs, configurer les clés d'accès MQTT et modifier le moteur de règles (Rule Engine) pour les alarmes.",
      url: "http://localhost:9090", // Port configuré dans docker-compose
      isInternal: false,
    ...
    },
    {
      ...
      description: "Supervision technique approfondie. Permet de requêter directement InfluxDB, d'analyser les patterns temporels et d'évaluer la précision des modèles de l'IA.",
      url: "http://localhost:3001", // Port configuré dans docker-compose
      isInternal: false,
      color: "from-amber-500 to-orange-600",
      icon: (
        <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 12l3-3 3 3 4-4M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
        </svg>
      )
    },
    {
      title: "Influxdb Data Explorer",
      description: "Base de données temporelle utilisée pour stocker les mesures des capteurs. Permet d'explorer les données brutes, de vérifier les écritures et d'optimiser les requêtes pour Grafana.",
  ...
  if (userRole === 'user') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-slate-100">
        ...
        <p className="text-slate-400 text-sm tracking-wide">Chargement de votre espace de travail...</p>
      </div>
    );
  }
      ...
      <header className="container mx-auto mb-12 flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-800 pb-6 gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm md:text-base">
            Portail d'accès centralisé aux infrastructures IoT et de supervision.
          </p>
        </div>
          ...
          {services.map((service, index) => (
            ...
            >
              <div>
                ...
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${service.color} flex items-center justify-center shadow-lg mb-6 group-hover:scale-110 transition-transform duration-300`}>
                  {service.icon}
                </div>
